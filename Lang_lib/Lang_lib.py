...
def crack(hashword,alphabet):
	...
	for hashletter in findall(".{64}",hashword.lower()):
		for letter in alphabet:
			if sha256(letter.encode()).hexdigest() == hashletter: result += letter
	return result
...
def delete(letter,text):
	...
	for char in chars:
		if char != letter: result += char
	return result


def dispersion(text,*count):
		'dispersion method can to dispersion text by your personal length or standard'
		characters,result = list(text) , []
		
		try: finalCount = count[0]
		except: finalCount = len(text)
			
		if  finalCount <= len(text):
			...
			return f"LengthError: {finalCount} is oversize of {text}\'s length !"
...
def repeatCount(letter,text):
	...
	for char in list(text):
		if char == letter: counter += 1
	return counter


def replace(x,text,y,CaseSensitivity,*From,**To):
	'replace x in text locate any y in your text. you can set case sensitivity to True or False , start point and finish point !'
	chars , result = list(text) , ""
	
	try: from_,to = From[0]-1,From[1]
	except: from_,to = 0,len(chars)
		
	for char in chars[from_:to]:
		if char == x and CaseSensitivity: result += y
		
		elif char.lower() == x.lower() and CaseSensitivity == False: result += y
...
def T_PDF(path,page,origin,destination) -> str:
	...
	PdfData = PDF(open(path,'rb')).getPage(page)
	return translate(PdfData.extractText(),origin,destination)
...
def T_DOCX(path,origin,destination) -> str:
	...
	DocxData = WORD(open(path,'rb'))
	return translate(DocxData,origin,destination)


def TTS(text,isSlow,path):
	'convert Text To Speech on audio files with slow speed or not.'
	from gtts import gTTS
	tts = gTTS(text=text,slow=isSlow)
	tts.save(path)
